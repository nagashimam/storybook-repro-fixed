...
class ProjectConverter {
    ...
    constructor({ host, projectName, eslintInitializer, }) {
        ...
        this.projectConfig = devkit_1.readProjectConfiguration(this.host, this.projectName);
        this.projectTSLintJsonPath = devkit_1.joinPathFragments(this.projectConfig.root, 'tslint.json');
        /**
        ...
        if (!host.exists(this.projectTSLintJsonPath)) {
            throw new Error(`We could not find a tslint.json for the selected project "${this.projectTSLintJsonPath}", maybe you have already migrated to ESLint?`);
        }
        this.rootTSLintJson = devkit_1.readJson(host, this.rootTSLintJsonPath);
        this.projectTSLintJson = devkit_1.readJson(host, this.projectTSLintJsonPath);
        /**
     ...
     */
    convertRootTSLintConfig(applyPackageSpecificModifications) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const convertedRoot = yield utils_1.convertTSLintConfig(this.rootTSLintJson, this.rootTSLintJsonPath, []);
            ...
            convertedRootESLintConfig.plugins = convertedRootESLintConfig.plugins.filter((p) => p !== '@typescript-eslint/tslint');
            /**
            ...
            if (nxEnforceModuleBoundariesRule) {
                devkit_1.updateJson(this.host, '.eslintrc.json', (json) => {
                    if (!json.overrides) {
                        return json;
                    }
            ...
            devkit_1.updateJson(this.host, '.eslintrc.json', (json) => {
                ...
                json.overrides = utils_1.deduplicateOverrides(json.overrides);
                return json;
            });
            /**
             ...
             */
            return utils_1.ensureESLintPluginsAreInstalled(this.host, convertedRoot.ensureESLintPlugins);
        });
    }
    convertProjectConfig(applyPackageSpecificModifications) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const convertedProjectConfig = yield utils_1.convertTSLintConfig(this.projectTSLintJson, this.projectTSLintJsonPath, 
            // Strip the extends on workspace tslint.json (see this util's docs for more info)
            [`${devkit_1.offsetFromRoot(this.projectConfig.root)}tslint.json`]);
            ...
            convertedProjectESLintConfig.plugins = convertedProjectESLintConfig.plugins.filter((p) => p !== '@typescript-eslint/tslint');
            const projectESLintConfigPath = devkit_1.joinPathFragments(this.projectConfig.root, '.eslintrc.json');
            /**
             * Apply updates to the new .eslintrc.json file for the project
             */
            devkit_1.updateJson(this.host, projectESLintConfigPath, (json) => {
                var _a;
             ...
             * of the project
             */
            convert_to_eslint_config_1.convertTSLintDisableCommentsForProject(this.host, this.projectName);
            /**
             * Through converting the config we may encounter TSLint rules whose closest
             * equivalent in the ESLint ecosystem comes from a separate package/plugin.
             *
             ...
             * explain that that's what we are doing.
             */
            return utils_1.ensureESLintPluginsAreInstalled(this.host, convertedProjectConfig.ensureESLintPlugins);
        });
    }
    removeProjectTSLintFile() {
        this.host.delete(devkit_1.joinPathFragments(this.projectConfig.root, 'tslint.json'));
    }
    isTSLintUsedInWorkspace() {
        const projects = devkit_1.getProjects(this.host);
    ...
    removeTSLintFromWorkspace() {
         ...
         */
        const uninstallTask = devkit_1.removeDependenciesFromPackageJson(this.host, [], ['tslint', 'codelyzer']);
        /**
         * Update global linter configuration defaults in workspace.json
         */
        const workspace = devkit_1.readWorkspaceConfiguration(this.host);
        this.cleanUpGeneratorsConfig(workspace);
        devkit_1.updateWorkspaceConfiguration(this.host, workspace);
        /**
        ...
        for (const [projectName, { generators }] of projects.entries()) {
            ...
            const project = devkit_1.readProjectConfiguration(this.host, projectName);
            this.cleanUpGeneratorsConfig(project);
            devkit_1.updateProjectConfiguration(this.host, projectName, project);
        }
    ...
    setDefaults(collectionName, removeTSLintIfNoMoreTSLintTargets) {
        const workspace = devkit_1.readWorkspaceConfiguration(this.host);
        workspace.generators = workspace.generators || {};
        workspace.generators[collectionName] =
            workspace.generators[collectionName] || {};
        const prev = workspace.generators[collectionName];
        workspace.generators = Object.assign(Object.assign({}, workspace.generators), { [collectionName]: Object.assign(Object.assign({}, prev), { 'convert-tslint-to-eslint': {
